...
def get_self_learning_data_OCT(fold, config):
    ...
    Cubes are generated by randomly cropping a 3D cube from the OCT data.
    One patient's cube may belong to different modalities (Pre, Post, Final).
    N cubes are then stacked together to form the final dataset. N x (C, H, W, D) where C is 1 (intensity).
